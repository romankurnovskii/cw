...
class EmployeesServiceTests {
	...
	private static final LocalDate BIRTH_DATE1 = LocalDate.ofYearDay(1980, 300);
	private static final int SALARY2 = 2000;
	private static final LocalDate BIRTH_DATE2 = LocalDate.ofYearDay(1990, 1);
	...
	Employee empl1 = new Employee(ID1, SALARY1, COMPANY1, BIRTH_DATE1, "name");
	Employee empl2 = new Employee(ID2, SALARY2, COMPANY1, BIRTH_DATE2, "name");
	Employee empl3 = new Employee(ID3, SALARY3, COMPANY2, BIRTH_DATE3, "name");
	Employee empl4 = new Employee(ID4, SALARY4, COMPANY2, BIRTH_DATE4, "name");
	Employee newEmployee = new Employee(123777, SALARY4, COMPANY2, BIRTH_DATE4, "name");
	Employee employees[] = { empl1, empl2, empl3, empl4 };
	EmployeesService employeesService;

	@BeforeEach
	void setUp() throws Exception {
		employeesService = new EmployeesMaps();
	...
	void testAddEmployee() {
		assertEquals(EmployeesReturnCodes.EMPLOYEE_ALREADY_EXISTS, employeesService.addEmployee(empl1));
		assertEquals(EmployeesReturnCodes.OK, employeesService.addEmployee(newEmployee));
	}

	@Test
	void testRemoveEmployee() {
		assertEquals(EmployeesReturnCodes.EMPLOYEE_NOT_FOUND, employeesService.removeEmployee(77777));
		Employee expected[] = { empl2, empl3, empl4 };
		assertEquals(EmployeesReturnCodes.OK, employeesService.removeEmployee(ID1));
	...
	void testGetEmployeesCompany() {
		...
		Iterable<Employee> employeesEmpty = employeesService.getEmployeesCompany("company");
		assertFalse(employeesEmpty.iterator().hasNext());
		Iterable<Employee> employeesCompany1 = employeesService.getEmployeesCompany(COMPANY1);
	...
	void testGetEmployeesAges() {
		...
		Iterable<Employee> employeesEmpty = employeesService.getEmployeesAges(100, 120);
		assertFalse(employeesEmpty.iterator().hasNext());
		Iterable<Employee> employees40Plus = employeesService.getEmployeesAges(40, 120);
	...
	void testGetEmployeesSalary() {
		Employee expected[] = { empl1, empl2 };
		Iterable<Employee> employeesEmpty = employeesService.getEmployeesSalary(10000, 25000);
		assertFalse(employeesEmpty.iterator().hasNext());
		Iterable<Employee> employees1000_2500 = employeesService.getEmployeesSalary(1000, 2500);
		testEmployees(expected, employees1000_2500);
	}

	@Test
	void testUpdateCompany() {
		assertEquals(EmployeesReturnCodes.OK, employeesService.updateCompany(ID1, "company"));
	...
	void testUpdateSalary() {
		...
		assertEquals(EmployeesReturnCodes.OK, employeesService.updateSalary(ID1, newSalary));
		Employee empl = employeesService.getEmployee(ID1);
		assertNotEquals(empl1, empl);
	...
	void testGroupingBySalary() {
		Map<String, List<Employee>> mapBySalary = employeesService.getEmployeesGroupedBySalary(1000);
